...
#' @noRd
.get_quota <- function(api_options) {
...
.get_data_meteocat <- function(api_options) {
  ...
  if (any(variables_statuses != 'OK')) {
    if (any(variables_codes == 429)) {
      messages_to_show <- variables_messages[which(variables_codes == 429)]
      message(copyright_style(messages_to_show[1]))
      Sys.sleep(60)
      return(.get_data_meteocat(api_options))
    } else {
      messages_to_show <- variables_messages[which(variables_codes != 200)]
      messages_to_show %>%
        purrr::walk(
  ...
  resolution_specific_unnest <- function(data) {
    data %>%
      purrr::map_dfr(function(variable_data) {
        tidyr::unnest(variable_data, cols = .data$variables, names_repair = 'universal') %>%
  ...
  ## TODO, print copyright and legal notice if any.
